...
def get_usfl_standings(season:int,apiKey:str,save=False):
    ...
    for i in json_data['standingsSections'][0]['standings']:
        ...
        for j in i['rows']:
            ...
            row_df = pd.DataFrame({'season':season,'division':division,'team_rank':team_rank,'team_analytics_name':team_analytics_name},index=[0])
            ...
            row_df[['overall_W','overall_L']] = row_df['overall_record_txt'].str.split('-',expand=True)
            row_df['overall_T'] = 0 # USFL should not have any ties, per their rulebook. 
            ...
            row_df['overall_point_diff'] = row_df['overall_points_scored'] - row_df['overall_points_allowed']
            
            row_df['home_record_txt'] = j['columns'][6]['text']
            row_df[['home_W','home_L']] = row_df['home_record_txt'].str.split('-',expand=True)
            row_df['home_T'] = 0

            try:
                row_df[['home_W','home_L']] = row_df[['home_W','home_L']].astype('int')
            except:
                print(f'No games played for the {team_analytics_name} in this season so far.')
            try:
                row_df['home_win_pct'] = (row_df['home_W'] + row_df['home_T']) / (row_df['home_W'] + row_df['home_L'] + row_df['home_T'])
                row_df['home_win_pct'] = row_df['home_win_pct'].round(3)
            except:
                row_df['home_win_pct'] = 0
            
            row_df['away_record_txt'] = j['columns'][7]['text']
            row_df[['away_W','away_L']] = row_df['away_record_txt'].str.split('-',expand=True)
            row_df['away_T'] = 0
            
            try:
                row_df[['away_W','away_L']] = row_df[['away_W','away_L']].astype('int')
                row_df['away_win_pct'] = (row_df['away_W'] + row_df['away_T']) / (row_df['away_W'] + row_df['away_L'] + row_df['away_T'])
                row_df['away_win_pct'] = row_df['away_win_pct'].round(3)
            except:
                row_df['away_win_pct'] = 0

            row_df['division_record_txt'] = j['columns'][8]['text']
            row_df[['division_W','division_L']] = row_df['division_record_txt'].str.split('-',expand=True)
            row_df['division_T'] = 0

            try:
                row_df[['division_W','division_L']] = row_df[['division_W','division_L']].astype('int')
                row_df['division_win_pct'] = (row_df['division_W'] + row_df['division_T']) / (row_df['division_W'] + row_df['division_L'] + row_df['division_T'])
...
def parse_usfl_player_stats(game_jsons:list,saveResults=False):
    ...
    pass_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','COMP','ATT','COMP%','PASS_YDS','PASS_TD','PASS_INT','NFL_QBR','YPA','YPC']
    try:
    ...
    passing_df.rename(columns={'PCT':'COMP%','YDS':'PASS_YDS','AVG':'YPA','TD':'PASS_TD','INT':'PASS_INT','QBR':'NFL_QBR'},inplace=True)
    ...
    rush_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','RUSH','RUSH_YDS','RUSH_AVG','RUSH_TD','RUSH_LONG']
    rush_df.rename(columns={'ATT':'RUSH','YDS':'RUSH_YDS','AVG':'RUSH_AVG','TD':'RUSH_TD','LNG':'RUSH_LONG'},inplace=True)
    rush_df[['RUSH','RUSH_YDS','RUSH_AVG','RUSH_TD','RUSH_LONG']] = rush_df[['RUSH','RUSH_YDS','RUSH_AVG','RUSH_TD','RUSH_LONG']].apply(pd.to_numeric)
    ...
    rec_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','REC_TARGETS','REC','REC_YDS','REC_AVG','REC_TD','REC_LONG','CATCH%','YDS/TARGET']
    receiving_df.rename(columns={'TGT':'REC_TARGETS','YDS':'REC_YDS','AVG':'REC_AVG','TD':'REC_TD','LNG':'REC_LONG'},inplace=True)
    receiving_df['REC_AVG'] = receiving_df['REC_AVG'].replace(['-'],None)
    receiving_df[['REC_TARGETS','REC','REC_YDS','REC_AVG','REC_TD','REC_LONG']] = receiving_df[['REC_TARGETS','REC','REC_YDS','REC_AVG','REC_TD','REC_LONG']].apply(pd.to_numeric)
    ...
    fum_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','FUMBLES','FUMBLES_LOST','FF','FR']
    fumbles_df.rename(columns={'FUM':'FUMBLES','LST':'FUMBLES_LOST','REC':'FR'},inplace=True)
    ...
    def_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','TOTAL','SOLO','AST','TFL','SACKS','INT','PD','DEF_TD']
    defensive_df.rename(columns={'TCK':'TOTAL','SOL':'SOLO','SCK':'SACKS','TD':'DEF_TD'},inplace=True)
    defensive_df[['TOTAL','SOLO','TFL','SACKS','INT','PD','DEF_TD']] = defensive_df[['TOTAL','SOLO','TFL','SACKS','INT','PD','DEF_TD']].apply(pd.to_numeric)
    ...
    kicking_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','FGM','FGA','FG%','FG_LONG','XPM','XPA','XP%']
    ...
    kr_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','KR','KR_YDS','KR_AVG','KR_TD','KR_LONG']
    kick_return_df.rename(columns={'RET':'KR','YDS':'KR_YDS','AVG':'KR_AVG','LNG':'KR_LONG','TD':'KR_TD'},inplace=True)
    kick_return_df[['KR','KR_YDS','KR_AVG','KR_TD','KR_LONG']] = kick_return_df[['KR','KR_YDS','KR_AVG','KR_TD','KR_LONG']].apply(pd.to_numeric)
    ...
    pr_column_names = ['season','game_id','game_date','team','team_nickname','loc','opponent','opponent_nickname','analytics_id','player_id','player_image','player_name','PR','PR_YDS','PR_AVG','PR_TD','PR_LONG']
    punt_return_df.rename(columns={'RET':'PR','YDS':'PR_YDS','AVG':'PR_AVG','LNG':'PR_LONG','TD':'PR_TD'},inplace=True)
    punt_return_df['PR_LONG'] = punt_return_df['PR_LONG'].replace(['-'],None)
    punt_return_df['PR_AVG'] = punt_return_df['PR_AVG'].replace(['-'],None)
    punt_return_df[['PR','PR_YDS','PR_AVG','PR_TD','PR_LONG']] = punt_return_df[['PR','PR_YDS','PR_AVG','PR_TD','PR_LONG']].apply(pd.to_numeric)
    ...
    main_df = main_df.sort_values(by=['season','game_date','game_id','loc','player_id'])

    if saveResults == True:
        min_season = int(main_df['season'].min())
        max_season = int(main_df['season'].max())
        for i in range(min_season,max_season+1):
            main_df.to_csv(f'player_stats/game_stats/{i}_player_game_stats.csv',index=False)
...
def main():
    ...
    get_usfl_rosters(2023,key,4,True)
